...
*)
  ...
  let weak_read_back env (nf : nf) : t =
    ...
    and weak_read_back_ env nf : t =
      weak_read_back_desc env nf.uid nf.desc
    and weak_read_back_desc env uid desc : t =
      let weak_read_back_no_force (Thunk (_local_env, t)) = t in
      match desc with
      | NVar v ->
          var (Option.get uid) v
      | NApp (nft, nfu) ->
          app ?uid (weak_read_back env nft) ~arg:(weak_read_back env nfu)
      | NAbs (_env, x, _t, nf) ->
          abs ?uid x (weak_read_back_no_force nf)
      | NStruct nstr ->
          str ?uid (Item.Map.map weak_read_back_no_force nstr)
      | NProj (nf, item) ->
          proj ?uid (read_back env nf) item
      | NLeaf -> leaf' uid
      | NComp_unit s -> comp_unit ?uid s
      | NoFuelLeft t -> t
