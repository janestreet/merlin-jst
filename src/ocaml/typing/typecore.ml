...
let collect_unknown_apply_args env funct ty_fun mode_fun rev_args sargs ret_tvar =
  ...
  let rec loop ty_fun mode_fun rev_args sargs =
    ...
    with Msupport.Resume ->
      Jkind.Sort.new_var (),
      Mode.Alloc.newvar (), newvar (Jkind.value ~why:(Unknown "merlin")),
...
<<<<<<< HEAD
      ...
      let level = get_level (instance ty_expected) in
      raise_error (error(loc_fun, env, err));
      { ty_arg = newvar2 level (Jkind.value ~why:(Unknown "merlin"))
      ; arg_mode = Mode.Alloc.newvar ()
      ; arg_sort = Jkind.Sort.new_var ()
      ; ty_ret = newvar2 level (Jkind.value ~why:(Unknown "merlin"))
...
>>>>>>> ups/501
  ...
  with exn ->
    ...
    let fake_label = {
      lbl_name = "";
      lbl_res = ty_exp;
      lbl_arg = newvar (Jkind.value ~why:(Unknown "merlin"));
      lbl_mut = Mutable;
      ...
      lbl_jkind = Jkind.value ~why:(Unknown "merlin");
    } in
...
=======
  partial_pred ~allow_modules:(Modules_allowed { scope = lev }) ~splitting_mode ~lev
>>>>>>> ups/501
