...
and type_function ?in_function loc attrs env (expected_mode : expected_mode)
  ...
  let (arg_mode, ty_arg, ret_mode, ty_res) =
    ...
    with Filter_arrow_failed err ->
      ...
      (Alloc_mode.newvar (),
       newvar2 level (Layout.value ~why:(Unknown "merlin")),
       Alloc_mode.newvar (),
       newvar2 level (Layout.value ~why:(Unknown "merlin")))
  in
  if has_local then
...
and type_label_access env srecord usage lid =
  ...
  with exn ->
    ...
    let fake_label = {
      lbl_name = "";
      lbl_res = ty_exp;
      lbl_arg = newvar (Layout.value ~why:(Unknown "merlin"));
      lbl_mut = Mutable;
      ...
      lbl_layout = Layout.value ~why:(Unknown "merlin");
    } in
...
and type_statement ?explanation ?(position=RNontail) env sexp =
  ...
  and tv = newvar (Layout.any ~why:Dummy_layout) in
