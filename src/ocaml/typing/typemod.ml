...
(*  TODO: check with Jacques why this is here
  ...
  let rec type_struct env sstr str_acc sig_acc sig_acc_include_functor =
    ...
    | pstr :: srem ->
        ...
        match type_str_item env pstr sig_acc_include_functor with
        | desc, sg, new_env ->
        ...
        | exception exn ->
            Msupport.raise_error exn;
            type_struct env srem str_acc sig_acc sig_acc_include_functor
  in
  Msupport.with_saved_types
    ?warning_attribute:
      (if Option.is_some toplevel || keep_warnings then None else Some [])
